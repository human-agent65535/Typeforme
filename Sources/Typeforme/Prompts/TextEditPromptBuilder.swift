...
enum TextEditPromptBuilder {
    static func build(
        for request: TextEditRequest,
        promptOverrideFolder: URL = AppSettings.promptOverrideFolder
    ) -> (system: String, user: String) {
        (systemPrompt(for: request, in: promptOverrideFolder), userPrompt(for: request))
    }

    private static func systemPrompt(for request: TextEditRequest, in promptOverrideFolder: URL) -> String {
        let modeInstruction: String
        switch request.intent {
        case .pinyinToChinese:
            return PromptOverrideStore.readPinyinPrompt(in: promptOverrideFolder)
                ?? BuiltInPrompts.pinyinToChinese
