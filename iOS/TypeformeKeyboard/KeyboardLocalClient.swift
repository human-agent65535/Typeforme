...
actor KeyboardLocalClient {
    ...
    func sendWithReachability(
        ...
        guard let bridgeToken,
            ...
            keyboardLocalClientLog.notice("reachable command skipped: missing bridge token action=\(command.action.rawValue, privacy: .public) command_id=\(command.id, privacy: .public)")
            KeyboardDiagnosticEventLog.record(
                source: "keyboard-local-client",
                event: "reachable_command_missing_token",
                fields: [
        ...
        let task = session.webSocketTask(with: urlRequest)
        task.maximumMessageSize = 1 * 1024 * 1024
        keyboardLocalClientLog.notice("reachable command starting action=\(command.action.rawValue, privacy: .public) command_id=\(command.id, privacy: .public) hello_timeout_ms=\(Int(helloTimeout * 1_000), privacy: .public) response_timeout_ms=\(Int(responseTimeout * 1_000), privacy: .public)")
        KeyboardDiagnosticEventLog.record(
            source: "keyboard-local-client",
            event: "reachable_command_starting",
            fields: [
                "action": command.action.rawValue,
                "command_id": command.id,
                "hello_timeout_ms": "\(Int(helloTimeout * 1_000))",
                "response_timeout_ms": "\(Int(responseTimeout * 1_000))",
            ]
        )
        task.resume()
        ...
        do {
            ...
            guard KeyboardLocalBridgeAuth.verifyServerHello(hello, bridgeToken: bridgeToken),
                ...
                task.cancel(with: .normalClosure, reason: nil)
                keyboardLocalClientLog.notice("reachable command unreachable: hello verification failed or client shutdown action=\(command.action.rawValue, privacy: .public) command_id=\(command.id, privacy: .public)")
                KeyboardDiagnosticEventLog.record(
                    source: "keyboard-local-client",
                    event: "reachable_command_unreachable_bad_hello",
                    fields: [
                        "action": command.action.rawValue,
                        "command_id": command.id,
                    ]
            ...
            keyboardLocalClientLog.notice("reachable command hello verified action=\(command.action.rawValue, privacy: .public) command_id=\(command.id, privacy: .public)")
            KeyboardDiagnosticEventLog.record(
                source: "keyboard-local-client",
                event: "reachable_command_hello_verified",
                fields: [
                    "action": command.action.rawValue,
                    "command_id": command.id,
                ]
            ...
            do {
                ...
                task.cancel(with: .normalClosure, reason: nil)
                guard shutdownGeneration == generation else { return .unreachable }
                keyboardLocalClientLog.notice("reachable command succeeded action=\(command.action.rawValue, privacy: .public) command_id=\(command.id, privacy: .public) state=\(status.state.rawValue, privacy: .public) status_command_id=\(status.commandID ?? "none", privacy: .public)")
                KeyboardDiagnosticEventLog.record(
                    source: "keyboard-local-client",
                    event: "reachable_command_succeeded",
                    fields: [
                        "action": command.action.rawValue,
                        "command_id": command.id,
                        "state": status.state.rawValue,
                ...
                task.cancel(with: .normalClosure, reason: nil)
                guard shutdownGeneration == generation else { return .unreachable }
                keyboardLocalClientLog.notice("reachable command reachable without status action=\(command.action.rawValue, privacy: .public) command_id=\(command.id, privacy: .public) error=\(error.localizedDescription, privacy: .public)")
                KeyboardDiagnosticEventLog.record(
                    source: "keyboard-local-client",
                    event: "reachable_command_reachable_without_status",
                    fields: [
                        "action": command.action.rawValue,
                        "command_id": command.id,
                        "error": error.localizedDescription,
            ...
            task.cancel(with: .normalClosure, reason: nil)
            keyboardLocalClientLog.notice("reachable command unreachable: hello failed action=\(command.action.rawValue, privacy: .public) command_id=\(command.id, privacy: .public) error=\(error.localizedDescription, privacy: .public)")
            KeyboardDiagnosticEventLog.record(
                source: "keyboard-local-client",
                event: "reachable_command_unreachable_hello_failed",
                fields: [
                    "action": command.action.rawValue,
                    "command_id": command.id,
                    "error": error.localizedDescription,
