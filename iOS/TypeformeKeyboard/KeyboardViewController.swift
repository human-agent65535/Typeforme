...
final class KeyboardViewController: UIInputViewController, UIGestureRecognizerDelegate, UIScrollViewDelegate {
    ...
    private struct KeyboardFocusPager {
        ...
        // Covers the tail of the gesture that changed surfaces. A fresh tap
        // after the swipe should not inherit that touch and commit a key.
    ...
    private var isAIWritingEnabled = false
    private var pinyinConversion = KeyboardPinyinConversionSession()
    private var pinyinConversionTask: Task<Void, Never>?
    ...
    private var voiceLeftControlGapConstraints: [NSLayoutConstraint] = []
    private var voiceRightControlGapConstraint: NSLayoutConstraint?
    ...
    private let keyboardFocusButton = HitInsetButton(type: .system)
    ...
    private let textKeyboardSwitchButton = HitInsetButton(type: .system)
    ...
    private var padRegularKeyRows: [UIStackView] = []
    ...
    private func textCharacterIntentPoint(from point: CGPoint) -> CGPoint {
        let verticalCorrection = currentKeyboardSurfaceMetrics.usesPadFullTextLayout
            ? 0
            : TextKeyboardTouchModel.characterIntentYCorrection
        return CGPoint(
            x: min(
                max(point.x - TextKeyboardTouchModel.characterIntentXCorrection, view.bounds.minX),
                view.bounds.maxX
            ),
            y: point.y - verticalCorrection
        )
    }

    private var isTouchLearningActiveForCurrentDevice: Bool {
        KeyboardTouchLearningAvailabilityPolicy.isActive(
            userEnabled: isTouchLearningEnabled,
    ...
    private func textKeyButtonBandTarget(in row: TextKeyboardHitRegion, at point: CGPoint) -> UIButton? {
        ...
        for index in buttons.indices {
            ...
            if isTouchLearningActiveForCurrentDevice,
    ...
    private func resolveGutterCandidate(
        ...
        guard isTouchLearningActiveForCurrentDevice else { return buttons[index].button }
    ...
    private func registerCommittedTextTouch(_ sample: TextKeyTouchSample) {
        guard isTouchLearningActiveForCurrentDevice else {
            pendingTextTouchSample = nil
    ...
    private func acceptPendingTextTouchIfSurvived(now: TimeInterval = Date().timeIntervalSince1970) {
        ...
        pendingTextTouchSample = nil
        guard isTouchLearningActiveForCurrentDevice else { return }
    ...
    private var currentKeyboardSurfaceMetrics: KeyboardSurfaceMetrics {
        KeyboardSurfaceLayoutPolicy.metrics(
            ...
            interfaceIdiomIsPad: traitCollection.userInterfaceIdiom == .pad,
            interfaceOrientationIsLandscape: keyboardInterfaceOrientationIsLandscape,
            screenShortestSide: Double(keyboardScreenShortestSide)
        )
    }

    private var keyboardInterfaceOrientationIsLandscape: Bool {
        let screenBounds = keyboardScreenBounds
        let surfaceWidth = viewIfLoaded?.bounds.width ?? 0
        let sceneOrientation = viewIfLoaded?.window?.windowScene?.interfaceOrientation
        // iPadOS 27's remote keyboard scene can keep reporting portrait after
        // rotation. A docked input view still exposes the authoritative span:
        // portrait is nearest the screen's short side and landscape the long.
        return KeyboardSurfaceLayoutPolicy.interfaceOrientationIsLandscape(
            surfaceWidth: Double(surfaceWidth),
            screenWidth: Double(screenBounds.width),
            screenHeight: Double(screenBounds.height),
            sceneOrientationIsLandscape: sceneOrientation?.isLandscape == true
        )
    }

    private var keyboardScreenShortestSide: CGFloat {
        let bounds = keyboardScreenBounds
        return min(bounds.width, bounds.height)
    }

    private var keyboardScreenBounds: CGRect {
        viewIfLoaded?.window?.windowScene?.screen.bounds ?? UIScreen.main.bounds
    }

    private var currentKeyboardContentHeight: CGFloat {
        let metrics = currentKeyboardSurfaceMetrics
        let usesTextHeight = keyboardFocus == .text
            && (renderedTextKeyboardLayoutKind?.isText ?? true)
        return CGFloat(usesTextHeight ? metrics.textContentHeight : metrics.voiceContentHeight)
    }
    ...
    private func applyKeyboardSurfaceMetricsForCurrentBounds() {
        ...
        heightConstraint?.constant = currentKeyboardContentHeight + Self.topChromeCoverHeight
    ...
    private func configureTopRow() {
        ...
        keyboardFocusButton.hitInsets = UIEdgeInsets(top: -5, left: -6, bottom: -5, right: -6)
    ...
    private func configureUtilityRow() {
        ...
        configureCapsuleButton(
            returnButton,
            title: showsVoiceDismissButton ? "" : "return",
            image: showsVoiceDismissButton ? "return" : nil,
            style: .utility
        )
        returnButton.widthAnchor.constraint(equalToConstant: showsVoiceDismissButton ? 44 : 78).isActive = true
    ...
    private func configureTextKeyboard() {
        ...
        textKeyboardSwitchButton.hitInsets = UIEdgeInsets(top: -5, left: -6, bottom: -5, right: -6)
    ...
    private func applyTextKeyBlockMetrics(for layoutKind: TextKeyboardLayoutKind) {
        ...
        keyRowsStack.spacing = isNumeric
            ...
            : textKeyVerticalGap
        keyRowsStack.distribution = !isNumeric && currentKeyboardSurfaceMetrics.usesPadNumberRow
            ? .fill
            : .fillEqually
    }
    ...
    private func addNumericBottomRow(decimalSeparator: String?, phoneSymbols: Bool) {
        ...
        if let decimalSeparator {
            ...
            let phoneSymbolsButton = makeTextKeyButton(title: "+*#", weight: .utility, sound: .modifier)
        ...
        let deleteKey = makeTextKeyButton(title: "", image: "delete.left", weight: .utility, sound: .delete)
    ...
    private func addPhoneSymbolBottomRow() {
        ...
        let numericButton = makeTextKeyButton(title: "123", weight: .utility, sound: .modifier)
    ...
    private func addPadSymbolKeyboardRows(_ rows: [[String]]) {
        ...
        if currentKeyboardSurfaceMetrics.usesPadNumberRow {
            ...
            let nativeRows = KeyboardTextLayoutPolicy.largePadSymbolRows(
                ...
                usesChinesePunctuation: usesChinesePunctuation
            )

            // Mirror the 13-inch native letter grid instead of composing four
            // unrelated symbol rows. The ordinary-key counts (13/13/11/10)
            // and utility widths now match Number, Q, A, and Z respectively,
            // so switching pages no longer makes every column jump sideways.
            addPadTextKeyRow(
                nativeRows[0],
                leadingButton: nil,
    ...
    private func addPadNumberRow() {
        ...
        keyRowsStack.addArrangedSubview(row)
        if let numberRowHeight = currentKeyboardSurfaceMetrics.padNumberRowHeight {
            let height = row.heightAnchor.constraint(equalToConstant: CGFloat(numberRowHeight))
    ...
    private func addPadTextKeyRow(
        ...
        let trailingSpacer: UIButton?
        if let trailingSpacerWidth {
            let spacer = UIButton(type: .custom)
            spacer.translatesAutoresizingMaskIntoConstraints = false
            spacer.backgroundColor = .clear
            spacer.isAccessibilityElement = false
            spacer.accessibilityElementsHidden = true
            row.addArrangedSubview(spacer)
            let width = spacer.widthAnchor.constraint(equalToConstant: trailingSpacerWidth)
            width.isActive = true
            keyboardRowConstraints.append(width)
        ...
        var boundaryButtons: [UIButton] = []
        if let leadingButton {
            directButtons.append(leadingButton)
            boundaryButtons.append(leadingButton)
        }
        boundaryButtons.append(contentsOf: keyButtons)
        if let trailingButton {
            directButtons.append(trailingButton)
            boundaryButtons.append(trailingButton)
        }
        if let trailingSpacer {
            directButtons.append(trailingSpacer)
            boundaryButtons.append(trailingSpacer)
        }
        registerTextKeyboardHitRow(
            row,
            routedButtons: keyButtons,
    ...
    private func registerPadRegularKeyRow(_ row: UIStackView) {
        if let referenceRow = padRegularKeyRows.first {
            let equalHeight = row.heightAnchor.constraint(equalTo: referenceRow.heightAnchor)
            equalHeight.isActive = true
            keyboardRowConstraints.append(equalHeight)
        }
        padRegularKeyRows.append(row)
    }
    ...
    private func addTextBottomRow(layout kind: KeyboardTextBottomLayoutKind) {
        ...
        if let returnKeyWidth = layout.returnKeyWidth {
            let weight = returnKeyWeight
            let returnKey = makeTextKeyButton(title: returnKeyTitle, image: returnKeyImageName, weight: weight, sound: .modifier)
    ...
    private func makeTextKeyButton(
        ...
        contentPlacement: KeyboardTextKeyContentPlacement? = nil,
        sound: KeyboardKeySound = .input
    ) -> KeyboardTextKeyControl {
        let button = KeyboardTextKeyControl(type: .system)
        button.pressSound = sound
        configureTextKeyButton(
            button,
            title: title,
    ...
    private func refreshCapsuleButtonConfigurations() {
        ...
        let usesPadVoiceUtilityIcons = traitCollection.userInterfaceIdiom == .pad
        returnButton.configuration = capsuleButtonConfiguration(
            title: usesPadVoiceUtilityIcons ? "" : "return",
            image: usesPadVoiceUtilityIcons ? "return" : nil,
            style: .utility
        )
    ...
    private func updateUI(animated: Bool = true) {
        ...
        let updates = {
            ...
            self.candidateScrollView.isUserInteractionEnabled = self.pinyinConversion.request == nil
            self.candidateGridScrollView.isUserInteractionEnabled = self.pinyinConversion.request == nil
    ...
    private func playKeyboardPressFeedbackIfNeeded(for control: UIControl) {
        let sound: KeyboardKeySound
        if control === deleteButton {
            sound = .delete
        } else if control === spaceButton || control === returnButton {
            sound = .modifier
        } else {
            guard control.isDescendant(of: keyRowsStack),
                  let key = control as? KeyboardTextKeyControl
            else { return }
            sound = key.pressSound
        }
        keyboardHaptics.playTextKeyPress(sound: sound)
    }
    ...
    private func applyKeyboardFocusChanges(isTextFocus: Bool) {
        ...
        // Voice First and the docked iPad text keyboard intentionally have
        // different heights. Re-assert the active surface here; trait-only
        // updates cannot observe a focus change.
        applyKeyboardHeightForCurrentTraits()
        updateKeyboardSurfaceMask()
        updateKeyboardOverlayOrdering()
        renderToolbarIcon(
    ...
    private func configureTextLanguageButton() {
        textLanguageButton.pressSound = .modifier
    ...
    private func handleTextSpace() {
        ...
        if !pendingRimeInput.isEmpty {
            if let input = pendingRimeInput.activeEngineInput,
        ...
        if currentRimeComposition.isComposing,
           rimeSpaceUsesRawLiteralBoundary(currentRimeComposition.input) {
            commitRawRimeInput(currentRimeComposition.input, appending: " ")
            resetShiftIfSticky()
            renderRefineSuggestionsIfIdle()
            return
        }
    ...
    private func startPinyinConversion() {
        ...
        lastBridgeContactAt = Date().timeIntervalSince1970
        showTextKeyboardStatus(
            NSLocalizedString("AI Writing…", comment: "Pinyin conversion progress"),
            duration: KeyboardBridgeCommandAction.refineText.requestTimeout
        )
        updateUI()
        let bridgeToken = hostKeyboardBridgeToken
        pinyinConversionTask = Task { [weak self] in
            guard let self else { return }
            do {
                let status = try await localClient.send(
                    command,
    ...
    private func failPinyinConversion(requestID: String) {
        guard pinyinConversion.request?.id == requestID else { return }
        cancelPinyinConversion()
        updateUI()
        showTextKeyboardStatus(
    ...
    private func cancelPinyinConversion() {
        guard let request = pinyinConversion.request else { return }
        pinyinConversion.cancel()
        pinyinConversionTask?.cancel()
        pinyinConversionTask = nil
        restoreKeyboardAfterPinyinConversion(requestID: request.id)
    }

    private func restoreKeyboardAfterPinyinConversion(requestID: String) {
        guard bridgeStatus?.commandID == requestID else { return }
        bridgeStatus = KeyboardBridgeStatus(
            commandID: requestID,
            state: .standby,
            message: "Ready",
            backendReachable: bridgeStatus?.backendReachable
        )
        textToolbarStatusClearTask?.cancel()
        textToolbarStatusClearTask = nil
        textToolbarStatusText = nil
    ...
    private var canStopActiveRefine: Bool {
        guard pinyinConversion.request == nil else { return false }
    ...
    @objc private func deletePressDown() {
        ...
        deleteRepeatTask = Task { [weak self] in
            ...
            while !Task.isCancelled {
                await MainActor.run {
                    self.keyboardHaptics.playTextKeyPress(sound: .delete)
    ...
    private func applyBridgeStatus(_ status: KeyboardBridgeStatus, recordsLiveContact: Bool = true) {
        // The dedicated AI response unlocks this marked range. A delayed host
        // standby snapshot must not re-enable keys while conversion is pending.
        if pinyinConversion.request != nil {
...
final class KeyboardTouchOverlayView: UIView {
    ...
    /// A resolved target belongs to the exact visible layout that produced
    /// it. Surface switches and row rebuilds must not reuse it against a new
    /// set of frames, even within the short accessibility activation window.
    func invalidateResolvedTarget() {
        clearPendingActivation()
    }
